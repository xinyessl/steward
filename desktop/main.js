...
function ptyCreate(e, { key, projectId, cwd, sessionId, engine }) {
  ...
  // 引擎:cmd=普通命令行终端(跑用户 shell)；codex 需真装了才走 codex，否则回退 claude
  const eng = engine === 'cmd' ? 'cmd' : ((engine === 'codex' && CODEX) ? 'codex' : 'claude');
...
setInterval(() => {
  ...
  for (const [key, r] of ptys) {
    if (r.engine === 'cmd') continue;   // 普通命令行终端：非 agent，不判忙/闲/确认
    if (!r.term) continue;
