...
2. 给项目开一个**终端窗口**，按起点二选一：
   - **已有代码库** → `/scan` 逆向扫源码，按功能模块生成 `docs/specs/*.md` 基线（草稿，待评审）。
   - **绿地（只有 PRD+原型、还没代码）** → PRD 放 `docs/PRD.md`、原型放 `docs/prototype/`，跑 `/init` 定栈 + 搭最小骨架，再 `/spec` 把 PRD 拆成 spec 树（此时 **PRD+原型 = 事实源**）。
